...
6. Deploy, authorize access to Sheets and email, then copy the `/exec` web-app URL.
7. In `index.html`, paste that URL into the `google-sheets-web-app-url` meta tag's `content` value.

The script creates a `Registrations` tab and stores each response as one row. Payment screenshots are validated by the browser but are intentionally not uploaded or stored; the **Payment Proof** column remains blank.
